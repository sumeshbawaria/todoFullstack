    ...
    return (
        <div>
            ...
            <div className={`group w-30 flex my-1 px-3 py-1 justify-start border-y border-transparent `}>
                <div>
                    <input type="checkbox" className='appearance-none h-3 w-3 rounded border border-gray-500 bg-transparent checked:border-blue-500 checked:bg-blue-500 cursor-default' onChange={toggleBox} checked={todo.completed} />
                </div>
                <input className={`w-40 ml-2 pl-2 ${todo.completed ? "line-through text-stone-400" : " "} overflow-hidden outline-0 border-0 cursor-default`} value={todo.task}  readOnly/>
            </div> 
            ...
            <div className={`group flex justify-evenly my-1 mx-4 px-3 py-1  border-y border-transparent ${!todo.completed &&"focus-within:border-gray-500" }`}>
                ...
                <input className={`w-fit ml-2 pl-2 grow ${todo.completed ? "cursor-default line-through text-stone-400" : " "} outline-0 border-0`} value={todo.task} readOnly={todo.completed} onChange={handleChange} onBlur={handleBlur} onKeyDown={handleKeyDown}/>
